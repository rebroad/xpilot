 ...
 *
 * $Id: T_Form.c,v 5.0 2001/04/07 20:01:00 dik Exp $
 */
   ...
   while (field != NULL) {
      if (field->active != INACTIVE ) {
         switch (field->type) {
            ...
            case T_TEXT_BUTTON:
            case T_HOLD_BUTTON:
               T_DrawTextButton(form->window,field->x,field->y,field->width,
                    field->height,RAISED,field->label);
               break;
               ...
               for (i=0;i<field->y2;i++) {
                  for (j=0;j<field->x2;j++) {
                     if (multi == (*(field->intvar)) ) {
                        T_DrawTextButton(form->window,j*w+field->x,
                             i*h+field->y,w-1,h-1,LOWERED,label);
                     } else {
                        T_DrawTextButton(form->window,j*w+field->x,
            ...
            case T_TEXT:
               T_DrawString(form->window,field->x,field->y,field->width,
                    field->height,BKGR,field->label,field->x2,CROP_RIGHT,-1);
               break;
            ...
            case T_STRING_ENTRY:
               ...
               if (field->label != NULL) {
                  ...
                  i = field->x+field->x2;
                  j = field->y+field->y2;
                  T_DrawString(form->window,i,j,w,field->height,BKGR,
                       field->label,JUSTIFY_LEFT,CROP_RIGHT,-1);
               }
               break;
         }
      }
   ...
   while (field != NULL) {
      if ( (x>field->x) && (y>field->y) && (x<(field->x+field->width)) &&
         ...
         switch (field->type) {
            ...
            case T_HOLD_BUTTON:
               T_DrawTextButton(form->window,field->x,field->y,field->width,
                    field->height,LOWERED,field->label);
               break;
            ...
            case T_MULTI_BUTTON:
               ...
               if ( ((multi+1) != (*(field->intvar))) || (field->null) ) {
                  ...
                  if ((*(field->intvar)) != 0) {
                     T_PopButton(form->window,x2*w+field->x,y2*h+field->y,
                        w-1, h-1, RAISED);
                  }
         ...
         switch (field->type) {
            ...
            case T_TEXT_BUTTON:
               ...
               while (event.type != ButtonRelease) {
                  ...
                  XCheckMaskEvent(display,(long)(ButtonReleaseMask), &event);
                  if (!XQueryPointer(display, event.xmotion.window, &root,
                       &child, &rootx, &rooty, &x, &y, &keysbuttons)) {
            ...
            case T_HOLD_BUTTON:
               ...
               while (event.type != ButtonRelease) {
                  count++;
                  CallFieldHandler(form,field,x,y,btn,count,field->handler);
                  if (XCheckMaskEvent(display,(long)(ButtonReleaseMask),
                       &event) == True) {
                     event.type = ButtonRelease;
                  }
               }
         ...
         switch (field->type) {
            ...
            case T_HOLD_BUTTON:
               ...
               if ((x>field->x) && (y>field->y) && (x<(field->x+field->width))
                  ...
                  CallFieldHandler(form, field, x, y, btn, 0, field->handler);
               }
               break;
...
#endif
      T_DrawString(form->window,field->x+5,field->y,field->width-10,
           field->height,BKGR,tmpstr,JUSTIFY_LEFT,CROP_RIGHT,
...
#endif
      T_DrawString(form->window,field->x+5,field->y,field->width-10,
           field->height,BKGR,field->charvar,JUSTIFY_LEFT,CROP_RIGHT,-1);
   }
}
...
void T_FormRedrawEntryField(char *charvar)
{
   ...
   while ( form != NULL ) {
      field = form->field;
      while ( field != NULL ) {
         if ( (field->type == T_STRING_ENTRY) && (field->charvar == charvar)
