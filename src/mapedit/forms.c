 ...
 *
 * $Id: forms.c,v 5.2 2001/05/20 14:10:39 millerjl Exp $
 */
...
void BuildPrefsForm(void)
{
   ...
   T_FormButton(prefwin,"Visibility",10,10+h*2,w-1,h-1,"Visibility, Teams...",OpenVisibilityPopup);
   T_FormButton(prefwin,"Cannons",10,10+h*3,w-1,h-1,"Cannons, Mines, Missiles...",OpenCannonsPopup);
   T_FormButton(prefwin,"Rounds",10,10+h*4,w-1,h-1,"Rounds, Connection...",OpenRoundsPopup);
   T_FormButton(prefwin,"InitItems",10,10+h*5,w-1,h-1,"Initial Items",OpenInitItemsPopup);
   T_FormButton(prefwin,"MaxItems",10,10+h*6,w-1,h-1,"Max. Items",OpenMaxItemsPopup);
   T_FormButton(prefwin,"Probs",10,10+h*7,w-1,h-1,"Probabilities",OpenProbsPopup);
   T_FormButton(prefwin,"Scoring",10,10+h*8,w-1,h-1,"Scoring",OpenScoringPopup);
...
void BuildPrefsSheet(int num)
{
   ...
   T_FormButton(*temp,"close_prefs",10,PREF_HEIGHT-10-PREF_BTN_HEIGHT,
        PREF_BTN_WIDTH,PREF_BTN_HEIGHT,"Close",FormCloseHandler);
   ...
   for (i=0; i<numprefs; i++) {
        ...
        switch (prefs[i].type) {
         case MAPWIDTH:
                T_FormStringEntry(*temp,"mapwidth",10+w+prefs[i].column*(w*2+20),
                                10+prefs[i].row*h+prefs[i].space+t,w,20,-w,0,
                                "Width:",map.width_str,3,ResizeWidth);
                break;

         case MAPHEIGHT:
                T_FormStringEntry(*temp,"mapheight",10+w+prefs[i].column*(w*2+20),
                                10+prefs[i].row*h+prefs[i].space+t,w,20,-w,0,
