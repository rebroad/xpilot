/* $Id: paintdata.cpp,v 1.14 2004/06/03 06:04:42 dick Exp $
 *
...
/*
 * $Log: paintdata.cpp,v $
 * Revision 1.14  2004/06/03 06:04:42  dick
 ...
 *
 * Revision 1.12  2004/02/03 03:39:52  dick
 * Add Windows precompiled header support.
 *
 * Revision 1.11  2004/01/20 15:14:27  dick
 ...
 *
 * Revision 1.7  2002/05/22 05:08:59  dick
...
void Erase_do_end(void)
{
    ...
    if (erp->num_rect != 0) {
	XFillRectangles(iniClient.dpy, p_draw, gc, erp->rect_ptr, erp->num_rect);
	UNEXPAND(erp->rect_ptr, erp->num_rect, erp->max_rect);
    }
    if (erp->num_arc != 0) {
	XDrawArcs(iniClient.dpy, p_draw, gc, erp->arc_ptr, erp->num_arc);
	UNEXPAND(erp->arc_ptr, erp->num_arc, erp->max_arc);
    }
    for (i = 0; i <= MAX_LINE_WIDTH; i++) {
	if (erp->num_seg[i] != 0) {
	    XSetLineAttributes(iniClient.dpy, gc, i,
			       LineSolid, CapProjecting, JoinMiter);
	    linewidth = true;
	    XDrawSegments(iniClient.dpy, p_draw, gc, erp->seg_ptr[i], erp->num_seg[i]);
...
void Rectangle_end(void)
{
	...
	if (num_rect[i] > 0) {
	    ...
	    rd.fillRectangles(iniClient.dpy, p_draw, gc, rect_ptr[i], num_rect[i]);
...
void Arc_end(void)
{
	...
	if (num_arc[i] > 0) {
	    SET_FG(colors[i].pixel);
	    rd.drawArcs(iniClient.dpy, p_draw, gc, arc_ptr[i], num_arc[i]);
...
void Segment_end(void)
{
	...
	if (num_seg[i] > 0) {
	    SET_FG(colors[i].pixel);
	    rd.drawSegments(iniClient.dpy, p_draw, gc,
		seg_ptr[i], num_seg[i]);
...
void Init_scale_array(void)
{
    ...
    if (iniClient.scaleFactor == 0.0)
	iniClient.scaleFactor = 1.0;
    if (iniClient.scaleFactor < 0.1)
	iniClient.scaleFactor = 0.1;
    if (iniClient.scaleFactor > 10.0)
	iniClient.scaleFactor = 10.0;
    scaleMultFactor = 1.0 / iniClient.scaleFactor;
