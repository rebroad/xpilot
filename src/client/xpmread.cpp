/* $Id: xpmread.cpp,v 1.5 2002/09/02 21:06:06 dick Exp $
 *
				...
				unsigned long *pixels)
{
	...
	for (k = key; k >= 0; k--) {
		...
		if (XParseColor(iniClient.dpy, DefaultColormap(iniClient.dpy, DefaultScreen(iniClient.dpy)),
				color_name, &xcolor)) {
		    break;
		}
	...
	if (!color_name) {
	    ...
	    for (j = 0; j < iniClient.maxColors; j++) {
...
static XImage *xpm_convert_to_image(XPM *xpm)
{
    ...
    img = XCreateImage(iniClient.dpy, visual, dispDepth, ZPixmap,
...
static Pixmap xpm_image_to_pixmap(XPM *xpm, XImage *img)
{
    ...
    pixmap = XCreatePixmap(iniClient.dpy, p_draw, xpm->width, xpm->height, dispDepth);
    if (pixmap != None) {
	pixgc = XCreateGC(iniClient.dpy, pixmap, 0, NULL);
	XPutImage(iniClient.dpy, pixmap, pixgc, img, 0, 0, 0, 0, xpm->width, xpm->height);
	XFreeGC(iniClient.dpy, pixgc);
    } else {
...
XImage *xpm_image_from_pixmap(Pixmap pixmap)
{
    ...
    if (!XGetGeometry(iniClient.dpy, pixmap, &rootw,
		      &x, &y,
		      &width, &height,
		      &border_width, &depth)) {
    ...
    img = XGetImage(iniClient.dpy, pixmap,
		    0, 0,
		    width, height,
		    AllPlanes, ZPixmap);
    if (!img) {
