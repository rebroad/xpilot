				...
				LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
	{
	    ...
	    Trace("MouseMove in %d %d/%d %s:%d\n", xidno,
		  LOWORD(lParam), HIWORD(lParam), xid[xidno].any.file,
		  xid[xidno].any.line);
    ...
    case WM_ERASEBKGND:
	{
		...
		if (hBmpDC) {
		    ...
		    FillRect(hBmpDC, &rect,
			     objs[xid[xidno].hwnd.bgcolor].brush);
		}
	    }
	    return (0);
	}
...
//    "#9f9f9f", "#5f5f5f", "#dfdfdf", "#202020"
    ...
    if (winmaxcolors < 4)
	error
	    ("I can't allocate 4 (colors or pens or brushes).  XPilot is probably not going to look very nice on this display");
...
//              myLogPal->palPalEntry[i].peFlags = PC_EXPLICIT;
	...
	if (!objs[i].brush) {
	    sprintf(s, "maxcolors=%d can't create brush %d", winmaxcolors,
		    i);
	    error(s);
	}
#else
	...
	if (!objs[i].pen) {
	    sprintf(s, "maxcolors=%d can't create pen %d", winmaxcolors,
		    i);
	    error(s);
	}
	objs[i].brush = CreateSolidBrush(WinXPColour(i));
    ...
    objs[WHITE + CLOAKCOLOROFS].pen =
	CreatePen(PS_DOT, 1, WinXPColour(WHITE));
    ...
    objs[BLUE + CLOAKCOLOROFS].pen =
	CreatePen(PS_DOT, 1, WinXPColour(BLUE));
...
static HPEN WinXMakePen(int cur_color, int Style, int Width)
{
	...
	if (Style = PS_DASH)
	    ...
	    dshs = dwcdashes;
	hPen =
	    ExtCreatePen(PS_GEOMETRIC | PS_USERSTYLE, Width, &lb, dshs[0],
			 &dshs[1]);
    } else
	hPen = CreatePen(Style, Width, WinXPColour(cur_color));
...
static void WinXSetPen(int xidno)
{
	...
	if (xid[xidno].hwnd.line_width > 2) {
		...
		objs[cur_color].fatpen =
		    WinXMakePen(WinXPColour(cur_color), PS_SOLID, 3);
	    hPen = objs[cur_color].fatpen;
	} else {
	    if (objs[cur_color].pen == NULL)
		objs[cur_color].pen =
		    WinXMakePen(WinXPColour(cur_color), PS_SOLID, 0);
	    hPen = objs[cur_color].pen;
	}
    } else {
	if (xid[xidno].hwnd.nodash) {
	    if (objs[cur_color].cdashpen == NULL)
		objs[cur_color].cdashpen =
		    WinXMakePen(cur_color, PS_DOT, 0);
	    hPen = objs[cur_color].cdashpen;
	} else {
	    if (objs[cur_color].dashpen == NULL)
		objs[cur_color].dashpen =
		    WinXMakePen(cur_color, PS_DASH, 0);
