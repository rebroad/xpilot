/* $Id: paintmap.cpp,v 1.9 2004/06/03 06:04:42 dick Exp $
 *
...
/*
 * $Log: paintmap.cpp,v $
 * Revision 1.9  2004/06/03 06:04:42  dick
...
void Paint_world(void)
{
    ...
    if (BIT(iniClient.instruments, SHOW_TEXTURED_WALLS)) {
	if (!wallTileReady) {
	...
	if (wallTileReady == 1) {
	    wallTileDoit = true;
	    XSetTile(iniClient.dpy, gc, wallTile);
	...
	for (rxb = xb; rxb <= xe; rxb++, xi++, x += BLOCK_SZ,
		...
		case SETUP_FILLED_NO_DRAW:
		    if (BIT(iniClient.instruments, SHOW_FILLED_WORLD|SHOW_TEXTURED_WALLS)
			&& fill_top_left == -1) {
		...
		case SETUP_DECOR_DOT_LD:
		    Gui_paint_decor_dot(x, y, iniClient.mapPointSize);
		    break;
		...
		if (!BIT(iniClient.instruments, SHOW_FILLED_WORLD|SHOW_TEXTURED_WALLS)) {
			...
			if (wallTileDoit) {
			    ...
			    SET_FG(colors[iniClient.wallColor].pixel);
			}
