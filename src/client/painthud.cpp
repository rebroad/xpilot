/* $Id: painthud.cpp,v 1.18 2007/01/17 21:35:15 dick Exp $
 *
...
/*
 * $Log: painthud.cpp,v $
 * Revision 1.18  2007/01/17 21:35:15  dick
 * Encapsulate all of the RobotWatch features into a RobotWatchMan object.
 *
 * Revision 1.17  2007/01/17 08:59:49  dick
 ...
 *
 * Revision 1.6  2002/05/22 05:08:59  dick
 * Update to XPilot r4_5_4alpha.
 *
 * Revision 1.5  2002/05/18 20:55:33  dick
...
#define METER_HEIGHT			8
	...
	if (!iniClient.blockBitmaps) {
		...
		rd.drawRectangle(iniClient.dpy, p_draw, gc,
	...
	rd.drawString(iniClient.dpy, p_draw, gc,
	...
	for (i=0; i < MAX_SCORE_OBJECTS; i++) {
		...
		if (sobj->count > 0) {
			if (sobj->count%3) {
				...
				if (wrap(&x, &y)) {
					SET_FG(colors[iniClient.hudColor].pixel);
...
{
	if (BIT(iniClient.instruments, SHOW_FUEL_METER))
		Paint_meter(-10, 20, "Fuel", (int)fuelSum, (int)fuelMax);
	if (BIT(iniClient.instruments, SHOW_POWER_METER) || control_count)
		Paint_meter(-10, 40, "Power", (int)displayedPower, (int)MAX_PLAYER_POWER);
	if (BIT(iniClient.instruments, SHOW_TURNSPEED_METER) || control_count)
		Paint_meter(-10, 60, "Turnspeed",
					(int)displayedTurnspeed, (int)MAX_PLAYER_TURNSPEED);
	if (control_count > 0)
		control_count--;
	if (BIT(iniClient.instruments, SHOW_PACKET_SIZE_METER))
		Paint_meter(-10, 80, "Packet",
				   (packet_size >= 4096) ? 4096 : packet_size, 4096);
	if (BIT(iniClient.instruments, SHOW_PACKET_LOSS_METER))
		Paint_meter(-10, 100, "Loss", packet_loss, FPS);
	if (BIT(iniClient.instruments, SHOW_PACKET_DROP_METER))
		Paint_meter(-10, 120, "Drop", packet_drop, FPS);
		if (BIT(iniClient.instruments, SHOW_PACKET_LAG_METER))
...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
	...
	int 		x, y;
	int 		i, dir = 96;
	int 		hudShipColor = iniClient.hudColor;
	Other*		target;
	ShipObj*	ship;
	char		str[50];
	 ...
	 */
	if (10 * iniClient.scaleFactor <= 15) {
		...
		if (iniClient.useErase){
			rd.drawLines(iniClient.dpy, p_draw, gc, points, i, 0);
			Erase_points(0, points, i);
		} else {
			rd.fillPolygon(iniClient.dpy, p_draw, gc,
				   points, i,
...
void Paint_hudradar(void)
{
	...
	for (i = 0; i < num_radar; i++) {
		...
		if ((sz & 0x80) == 0) {
			...
			int x = (int)(radar_ptr[i].x * hrscale - 
				(world.x + ext_view_width / 2) * xf);

			int y = (int)(radar_ptr[i].y * hrscale - 
...
void Paint_HUD(void)
{
	 ...
	 * Show speed pointer
	 */
	if (iniClient.ptrMoveFact != 0.0
		&& selfVisible != 0
		&& (vel.x != 0 || vel.y != 0)) {
		Segment_add(iniClient.hudColor,
					ext_view_width / 2,
					ext_view_height / 2,
					(int)(ext_view_width / 2 - iniClient.ptrMoveFact*vel.x),
					(int)(ext_view_height / 2 + iniClient.ptrMoveFact*vel.y));
	}

		if (BIT(iniClient.instruments, SHOW_HUD_RADAR)) 
				Paint_hudradar();

	if (!BIT(iniClient.instruments, SHOW_HUD_INSTRUMENTS)) {
		return;
	}
	...
	hud_pos_x = (int)(ext_view_width / 2 - iniClient.hudMoveFact*vel.x);
	hud_pos_y = (int)(ext_view_height / 2 + iniClient.hudMoveFact*vel.y);

	/* HUD frame */
	gcv.line_style = LineOnOffDash;
	XChangeGC(iniClient.dpy, gc, GCLineStyle | GCDashOffset, &gcv);

	if (BIT(iniClient.instruments, SHOW_HUD_HORIZONTAL)) {
		rd.drawLine(iniClient.dpy, p_draw, gc,
	...
	if (BIT(iniClient.instruments, SHOW_HUD_VERTICAL)) {
		rd.drawLine(iniClient.dpy, p_draw, gc,
	...
	XChangeGC(iniClient.dpy, gc, GCLineStyle, &gcv);
	...
	for (i = 0; i < NUM_ITEMS; i++) {
		...
		if (BIT(iniClient.instruments, SHOW_ITEMS)) {
			lastNumItems[i] = num;
			if (num <= 0)
				num = -1;
		} else {
			if (num != lastNumItems[i]) {
				numItemsTime[i] = (int)(iniClient.showItemsTime * (float)FPS);
				lastNumItems[i] = num;
			}
			if (numItemsTime[i]-- <= 0) {
				numItemsTime[i] = 0;
	...
	if (fuelCount || fuelSum < iniClient.fuelLevel3) {
		did_fuel = 1;
	...
	for (i=0, j=0; i < MAX_SCORE_OBJECTS; i++) {
		...
		if (sobj->hud_msg_len > 0) {
			if (j == 0 &&
				...
				(did_fuel || BIT(iniClient.instruments, SHOW_HUD_VERTICAL)))
						++j;
			rd.drawString(iniClient.dpy, p_draw, gc,
	...
	if (BIT(iniClient.instruments, SHOW_FUEL_GAUGE) == 0
		|| !((fuelCount)
		   || (fuelSum < iniClient.fuelLevel3
			  && ((fuelSum < iniClient.fuelLevel1 && (loops%4) < 2)
				  || (fuelSum < iniClient.fuelLevel2
					  && fuelSum > iniClient.fuelLevel1
					  && (loops%8) < 4)
				  || (fuelSum > iniClient.fuelLevel2)))))
		return;
	...
	rd.fillRectangle(iniClient.dpy, p_draw, gc,
				   WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET 
						+ FUEL_GAUGE_OFFSET) + 1,
				   WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET 
...
void Paint_messages(void)
{
	...
	if (charsPerTick <= 0.0)
		charsPerTick = (float)iniClient.charsPerSecond / FPS;
	...
	if (iniClient.selectionAndHistory) {
		while (last_msg_index < iniClient.maxMessages
	...
	for (i = (BIT(iniClient.instruments, SHOW_REVERSE_SCROLL) ? 2 * iniClient.maxMessages - 1 : 0);
		 (BIT(iniClient.instruments, SHOW_REVERSE_SCROLL) ? i >= 0 : i < 2 * iniClient.maxMessages);
		 i += (BIT(iniClient.instruments, SHOW_REVERSE_SCROLL) ? -1 : 1)) {
		if (i < iniClient.maxMessages) {
			msg = TalkMsg[i];
		} else {
			msg = GameMsg[i - iniClient.maxMessages];
		}
		if (msg->len == 0)
			continue;
		...
		if (i < iniClient.maxMessages) {
			...
			if (!BIT(iniClient.instruments, SHOW_MESSAGES)) {
				continue;
			}
			x = BORDER;
			y = bot_y;
		...
		if (msg->life > MSG_FLASH) {
			...
			msg_color = iniClient.oldMessagesColor;
		}
		 ...
		 */
		if (iniClient.selectionAndHistory && selection.draw.state == SEL_EMPHASIZED
			&& i < iniClient.maxMessages
			...
			if (ptr) {
				XSetForeground(iniClient.dpy, messageGC, colors[msg_color].pixel);
				rd.drawString(iniClient.dpy, p_draw, messageGC, x + xoff, y, ptr, l);
			}
			if (ptr2) {
				XSetForeground(iniClient.dpy, messageGC, colors[DRAW_EMPHASIZED].pixel);
				rd.drawString(iniClient.dpy, p_draw, messageGC, x + xoff2, y, ptr2, l2);
			}
			if (ptr3) {
				XSetForeground(iniClient.dpy, messageGC, colors[msg_color].pixel);
				rd.drawString(iniClient.dpy, p_draw, messageGC, x + xoff3, y, ptr3, l3);
			}
		...
		{
			XSetForeground(iniClient.dpy, messageGC, colors[msg_color].pixel);
			rd.drawString(iniClient.dpy, p_draw, messageGC, x, y, msg->txt, len);
		}
		...
		if (!strncmp(msg->txt, "http://", 7))	// underline urls
		{
			rd.drawLine(iniClient.dpy, p_draw, messageGC,
					  x, y,
					  x+width, y);
			urlRects[i].Set(x, y-SPACING, x+width, y);
		}
 ...
 */
static void _AddMessage(PCSTR message)
{
	int 				i, len;
	...
	show_reverse_scroll = BIT(iniClient.instruments, SHOW_REVERSE_SCROLL);
#endif

	len = strlen(message);
	if (message[len - 1] == ']' || strncmp(message, " <", 2) == 0) {
#ifndef _WINDOWS
		if (iniClient.selectionAndHistory && selection.draw.state == SEL_PENDING) {
...
#ifndef _WINDOWS
	if (iniClient.selectionAndHistory && is_drawn_talk_message) {
		 ...
		 */
		if (show_reverse_scroll && last_msg_index == iniClient.maxMessages - 1) {
			scrolling = true;
		}
	...
	tmp = msg_set[iniClient.maxMessages - 1];
	for (i = iniClient.maxMessages - 1; i > 0; i--) {
	...
	if (iniClient.selectionAndHistory && is_drawn_talk_message
		...
		if ((scrolling && selection.draw.y2 == 0)
			  || (!show_reverse_scroll && selection.draw.y1 == iniClient.maxMessages - 1)) {
			/*
			...
			if (scrolling) {
				...
				selection.draw.y1++;
				if (selection.draw.y2 == iniClient.maxMessages - 1) {
	 ...
	 */
	if (iniClient.messagesToStdout == 2 
	  || (iniClient.messagesToStdout == 1
	  && message[0] 
...
static void	_AddMessage(PCSTR* lpszFormat, va_list args, ...)
{
	char	u[MSG_LEN*2];
	char*	s = u;

	vsnprintf(u, MSG_LEN*2, *lpszFormat, args);
	va_end(args);
	_AddMessage(u);
}

///////////////////////////////////////////////////////////////////////////////
void	AddMessage(PCSTR lpszFormat, ...)
{
    va_list	 ap;			/* Argument pointer */
	va_start(ap, lpszFormat);
	_AddMessage(&lpszFormat, ap);
}
...
void Delete_pending_messages(void)
{
	...
	for (i = 0; i < iniClient.maxMessages; i++) {
...
void Add_pending_messages(void)
{
	...
	for (i = iniClient.maxMessages-1; i >= 0; i--) {
		if (TalkMsg_pending[i]->len > 0) {
			AddMessage(TalkMsg_pending[i]->txt);
		}
		if (GameMsg_pending[i]->len > 0) {
			AddMessage(GameMsg_pending[i]->txt);
		}
	}
...
void Paint_recording(void)
{
	...
	XDrawString(iniClient.dpy, p_draw, gc, x, y, buf, len);
