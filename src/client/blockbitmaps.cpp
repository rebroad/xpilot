/* $Id: blockbitmaps.cpp,v 1.7 2004/02/03 03:39:52 dick Exp $
 *
 ...
 */
/*
 * $Log: blockbitmaps.cpp,v $
 * Revision 1.7  2004/02/03 03:39:52  dick
 * Add Windows precompiled header support.
 *
 * Revision 1.6  2004/01/09 05:00:00  dick
 * Don't try to paint a bitmap meter, if the bitmap isn't ready
 *
 ...
 *
 * Revision 1.4  2002/09/02 21:06:06  dick
 * Encapsulate dpy and kdpy.  Singleton ini becomes iniClient.
 *
 * Revision 1.3  2002/05/18 20:55:33  dick
		 ...
		 int number)
{
	...
	Block_bitmap_create(iniClient.dpy, d, &xp_pixmaps[type], number, width, height);
	scaled_bitmaps++;
    }
		    ...
		    int image, int size)
{
	...
	Block_bitmap_create(iniClient.dpy, d, &xp_pixmaps[type], image, width, height);
	scaled_bitmaps++;
    }
...
void Cache_ships(Drawable d)
{
	...
	for (i=0; i < 3 - scaled_bitmaps;i++) {
		...
		!= WINSCALE(32))
	    {
    		Block_bitmap_create(iniClient.dpy, d,
			       ...
			       int width, int height)
{
	...
	XFreePixmap(iniClient.dpy, xp_pixmap->bitmaps[image].bitmap);
	xp_pixmap->bitmaps[image].bitmap = None;
    }
    if (xp_pixmap->bitmaps[image].mask) {
	XFreePixmap(iniClient.dpy, xp_pixmap->bitmaps[image].mask);
	xp_pixmap->bitmaps[image].mask = None;
    }

    pixmap = XCreatePixmap(iniClient.dpy, d,
			   width, height,
			   dispDepth);
    if (!pixmap) {
			    ...
			    RGB_COLOR color)
{
    ...
    pixel = (RGB)(r, g, b);
    SET_FG(pixel);
    XDrawPoint(iniClient.dpy, xp_pixmap->bitmaps[image].bitmap, gc,
	       x, y);

    pixel = (color) ? 1 : 0;
    XSetForeground(iniClient.dpy, maskGC, pixel);
    XDrawPoint(iniClient.dpy, xp_pixmap->bitmaps[image].mask, maskGC,
	       x, y);
}
				   ...
				   int image, int size)
{
    ...
    */
    XCopyArea(iniClient.dpy, xp_pixmaps[type].bitmaps[image].bitmap,
	      d, gc,
			      ...
			      int size)
{
    ...
    XCopyArea(iniClient.dpy, xp_pixmaps[type].bitmaps[1].bitmap, /* 1 = filled image */
	      d, gc,
	      0, 0,
			...
			int height, int number)
{
    ...
    XSetClipOrigin(iniClient.dpy, gc, x, y);
    XSetClipMask(iniClient.dpy, gc, bit->mask);
    XCopyArea(iniClient.dpy, bit->bitmap,
	      d, gc,
	      0 + box->xmin, 0 + box->ymin,
	      box->xmax + 1 - box->xmin, box->ymax + 1 - box->ymin,
	      x + box->xmin, y + box->ymin);
    XSetClipMask(iniClient.dpy, gc, None);
}
