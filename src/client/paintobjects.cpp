/* $Id: paintobjects.cpp,v 1.9 2004/06/03 06:04:42 dick Exp $
 *
...
void Paint_item_symbol(u_byte type, Drawable d, GC mygc, int x, int y, int color)
{
    ...
    gcv.ts_x_origin = x;
    gcv.ts_y_origin = y;
    XChangeGC(iniClient.dpy, mygc,
	      GCStipple|GCFillStyle|GCTileStipXOrigin|GCTileStipYOrigin,
	      &gcv);
    rd.paintItemSymbol(type, d, mygc, x, y, color);
    XFillRectangle(iniClient.dpy, d, mygc, x, y, ITEM_SIZE, ITEM_SIZE);
...
void Paint_item(u_byte type, Drawable d, GC mygc, int x, int y)
{
    ...
    rd.drawLines(iniClient.dpy, d, mygc, points, 4, CoordModeOrigin);
#endif
    ...
    str[1] = '\0';
    rd.drawString(iniClient.dpy, d, mygc,
...
static void Paint_mines(void)
{
	...
	for (i = 0; i < num_mine; i++) {
		 ...
		 */
		name = NULL;
		if (BIT(iniClient.instruments, SHOW_MINE_NAME)) {
		    if (mine_ptr[i].id != 0) {
			Other*	other;
...
static void Paint_wreckages(void)
{
	...
	for  (i = 0; i < num_wreckage; i++) {
	    ...
	    y = wreckage_ptr[i].y;
	    if (wrap(&x, &y)) {
		deadly = (wreckage_ptr[i].wrecktype & 0x80) != 0;
