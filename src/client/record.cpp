/* $Id: record.cpp,v 1.11 2004/02/03 03:39:52 dick Exp $
 *
...
static void RWriteString(PCSTR str)
{
    int				len = strlen(str);
    int				i;
...
static void RWriteHeader(void)
{
    ...
    RWriteString(iniClient.name.s);
    RWriteString(iniClient.realname.s);
    RWriteString(hostname);
    RWriteString(servername);
    RWriteByte((uchar)FPS);
    time(&t);
	...
	*ptr = '\0';
    ...
    putc(iniClient.maxColors, recordFP);
    ...
    RWriteString(iniClient.gameFontName);
    RWriteString(iniClient.messageFontName);
...
static int RGetPixelIndex(unsigned long pixel)
{
    ...
    for (i = 1; i < iniClient.maxColors; i++) {
	if (pixel == (colors[BLACK].pixel ^ colors[i].pixel)) {
	    return i + iniClient.maxColors;
	}
    }
...
static void RWriteTile(Pixmap tile)
{
	...
	for (x = 0; x < img->width; x++) {
	    ...
	    for (i = 0; i < iniClient.maxColors - 1; i++) {
		if (pixel == colors[i].pixel) {
		    break;
		}
	    }
...
static void RWriteGC(GC gc, unsigned long req_mask)
{
	...
	XGetGCValues(iniClient.dpy, gc, write_mask, &values);
...
static void REndFrame(void)
{
	...
	XGetGCValues(iniClient.dpy, gc, GCForeground, &values);

	RWriteByte(RC_DAMAGED);
	if ((damaged & 1) != 0) {
	    XSetForeground(iniClient.dpy, gc, colors[BLUE].pixel);
	} else {
	    XSetForeground(iniClient.dpy, gc, colors[BLACK].pixel);
	}
	RWriteGC(gc, GCForeground | RTILEGC);
	RWriteByte((uchar)damaged);

	XSetForeground(iniClient.dpy, gc, values.foreground);
    }
			   ...
			   XRectangle *rectangles, int nrectangles)
{
	...
	for (i = 0; i < nrectangles; i++) {
	    ...
	    RWriteByte((uchar)rectangles[i].width);
	    RWriteByte((uchar)rectangles[i].height);
	}
    }
    return 0;
}
		     ...
		     XArc *arcs, int narcs)
{
	...
	for (i = 0; i < narcs; i++) {
	    ...
	    RWriteByte((uchar)arcs[i].width);
	    RWriteByte((uchar)arcs[i].height);
...
/* No recording available with PENS_OF_PLENTY under Windows.
*/
