/* $Id: welcome.cpp,v 1.22 2004/02/03 03:39:52 dick Exp $
 *
...
/*
 * $Log: welcome.cpp,v $
 * Revision 1.22  2004/02/03 03:39:52  dick
 * Add Windows precompiled header support.
 *
 * Revision 1.21  2004/01/06 04:48:27  dick
 ...
 *
 * Revision 1.20  2002/09/16 22:16:52  dick
 * Stringize conpar.  NOTE: this breaks "join internet server".
 *
 * Revision 1.19  2002/09/02 21:06:06  dick
 * Encapsulate dpy and kdpy.  Singleton ini becomes iniClient.
 *
 * Revision 1.18  2002/08/28 01:42:19  dick
 * Retab
 *
 * Revision 1.17  2002/08/01 15:14:10  dick
 * server_name becomes serverName, etc.
 *
 * Revision 1.16  2002/05/22 13:19:46  dick
 * Update to XPilot r4_5_4alpha
 *
 * Revision 1.15  2002/05/22 05:08:59  dick
 * Update to XPilot r4_5_4alpha.
 *
 * Revision 1.14  2002/05/18 22:51:58  dick
 * Linux cleanups
 *
 * Revision 1.13  2002/05/18 20:55:33  dick
...
//////////////////////////////////////////////////////////////////////////////
// Everything below here is not on Windows
#if defined(_UNIX) || defined(_CYGWIN)
...
static Connectparam*	localnet_conpars = 0;
...
static struct Meta		metas[NUM_METAS] = {
								{ META_HOST,	 META_IP,	  MetaConnecting },
								{ META_HOST_TWO, META_IP_TWO, MetaConnecting }
						};
...
static void Welcome_process_exposure_events(void)
{
	...
	while (XCheckMaskEvent(iniClient.dpy, ExposureMask, &event)) {
...
static int Localnet_cb(int widget, void *user_data, const char **text)
{
	...
	if (n > 0) {
		...
		// localnet_conpars = (Connectparam *) malloc(n * sizeof(Connectparam));
		localnet_conpars = new Connectparam[n];
		if (!localnet_conpars) {
			Not_enough_memory();
			free(server_names);
			free(server_addrs);
		...
		for (i = 0; i < n; i++) {
			...
			localnet_conpars[i].serverName = name_ptrs[i];
			localnet_conpars[i].serverAddr = addr_ptrs[i];
			...
			button =
				Widget_create_label(subform_widget,
									...
									1, localnet_conpars[i].serverName);
...
static void Meta_connect(int *connections_ptr, int *maxfd_ptr)
{
	...
	for (i = 0; i < NUM_METAS; i++) {
		...
		status = metas[i].sock.OpenTcpConnectedNonBlocking(metas[i].addr,
														   META_PROG_PORT);
...
static void Ping_servers(void)
{
	...
	do {
		while (outstanding < (ms / interval + 1)) {
			...
			it_sip = SI_DATA(it);
			sock.SendDest(it_sip->ip_str, it_sip->port,
						   sbuf.buf, sbuf.len);
...
static int Internet_server_join_cb(int widget, void *user_data, const char **text)
{
	...
	Connectparam*		conpar = &connect_param;
	int 				result;

	/* structure copy */
	*conpar = *global_conpar;
	conpar->serverName = sip->hostname;
	conpar->serverAddr = sip->ip_str;
	conpar->contactPort = sip->port;
/* DIK: FIXME
	result = Contact_servers(1, conpar->serverAddr, 1, 0, 0, NULL,
							 0, NULL,
							 NULL, NULL,
							 conpar);
*/
	if (result) {
		...
		*global_conpar = *conpar;
		...
		printf("Server %s (%s) didn't respond on port %d\n",
				(PCSTR)conpar->serverName, (PCSTR)conpar->serverAddr, conpar->contactPort);
	}
...
static int Welcome_process_pending_events(Connectparam *conpar)
{
	...
	while (XEventsQueued(iniClient.dpy, QueuedAfterFlush) > 0) {
...
#if 0
	XSynchronize(iniClient.dpy, True);
#endif
	if (Init_top() == -1) {
		return -1;
	}
