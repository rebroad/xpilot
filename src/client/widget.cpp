/* $Id: widget.cpp,v 1.16 2004/04/26 06:59:42 dick Exp $
 *
...
/*
 * $Log: widget.cpp,v $
 * Revision 1.16  2004/04/26 06:59:42  dick
 * Widget_destroy_children can clean out a pulldown menu
 *
 * Revision 1.15  2004/04/25 05:44:05  dick
 * Retab
 *
 * Revision 1.14  2004/04/23 17:31:30  dick
 * Retab
 *
 * Revision 1.13  2004/02/29 00:07:42  dick
 * Don't try to XFlush a WIDGET_DUMMY
 *
 * Revision 1.12  2004/02/28 07:44:38  dick
 * Comment out unused typedef
 *
...
static void Widget_window_gravity(Window w, int gravity)
{
	...
	XChangeWindowAttributes(iniClient.dpy, w, mask, &attr);
}
...
void Widget_destroy_children(int widget_desc)
{
	...
	if ((w = Widget_pointer(widget_desc)) != NULL) {
		if (w->type == WIDGET_BUTTON_MENU)
		{
			menuw = (widget_menu_t*)w->sub;
			Widget_destroy(menuw->pulldown_desc);
...
void Widget_destroy(int widget_desc)
{
	...
	if ((w = Widget_pointer(widget_desc)) != NULL) {
		...
		if (w->window != 0) {
			XDestroyWindow(iniClient.dpy, w->window);
			w->window = 0;
		}
						 ...
						 int width, int height, void *sub)
{
	...
	if ((widget = Widget_new(&desc)) != NULL) {
		...
		XDestroyWindow(iniClient.dpy, window);
	}
...
int Widget_resize(int widget_desc, int width, int height)
{
	...
	XResizeWindow(iniClient.dpy, widget->window, width, height);
	widget->width = width;
	widget->height = height;
	return widget_desc;
}
...
static void Widget_draw_button(widget_t *widget, bool inverse, const char *label)
{
	...
	XSetForeground(iniClient.dpy, buttonGC, bg);
	XFillRectangle(iniClient.dpy, widget->window, buttonGC,
				   0, 0, widget->width, widget->height);
	XSetForeground(iniClient.dpy, buttonGC, colors[buttonColor].pixel);
	...
	ShadowDrawString(iniClient.dpy, widget->window, buttonGC,
...
{
	XClearWindow(iniClient.dpy, widget->window);
		IFWINDOWS( Trace("Widget_draw_input: w=%d <%s>\n", widget->window, str); )
	XDrawString(iniClient.dpy, widget->window, textGC,
				(widget->width
...
static void Widget_draw_arrow(widget_t *widget)
{
	...
	XSetForeground(iniClient.dpy, buttonGC, colors[bg].pixel);
	XFillRectangle(iniClient.dpy, widget->window, buttonGC,
				   0, 0,
				   widget->width, widget->height);
	XSetForeground(iniClient.dpy, buttonGC, colors[fg].pixel);
	pts[0].x = left;
	...
	pts[3].x = pts[0].x;
	pts[3].y = pts[0].y;
	XFillPolygon(iniClient.dpy, widget->window, buttonGC, pts, 4,
...
static void Widget_draw_slider(widget_t *widget)
{
	...
	XSetForeground(iniClient.dpy, buttonGC, colors[RED].pixel);
	if (widget->type == WIDGET_SLIDER_HORI) {
		...
		XFillRectangle(iniClient.dpy, widget->window, buttonGC,
					   ...
					   block_size, unit / 2);
		XSetForeground(iniClient.dpy, buttonGC, colors[BLACK].pixel);
		XFillRectangle(iniClient.dpy, widget->window, buttonGC,
					   block_offset, unit / 4,
					   block_start, unit / 2);
		XFillRectangle(iniClient.dpy, widget->window, buttonGC,
...
static void Widget_viewer_draw_lines(widget_t *widget, int start, int end)
{
	...
	for (i = start; i < end; i++) {
		if (!iniClient.mono) {
			XSetForeground(iniClient.dpy, motdGC, colors[BLACK].pixel);
			XDrawString(iniClient.dpy, widget->window, motdGC,
						x+1, y+1,
						(char *) viewerw->line[i].txt, viewerw->line[i].len);
			XSetForeground(iniClient.dpy, motdGC, colors[WHITE].pixel);
			XDrawString(iniClient.dpy, widget->window, motdGC,
						x-1, y-1,
						(char *) viewerw->line[i].txt, viewerw->line[i].len);
		}
...
static void Widget_draw_expose(int widget_desc, XExposeEvent *expose)
{
	...
	case WIDGET_LABEL:
		...
		ShadowDrawString(iniClient.dpy, widget->window, textGC,
...
static void Widget_button_slider(XEvent *event, widget_t *widget, bool pressed)
{
	...
	if (widget->type == WIDGET_SLIDER_VERT) {
		...
		if (block_new_start != block_start) {
			...
			XMoveWindow(iniClient.dpy, viewer_widget->window,
...
static void Widget_button_motion(XEvent *event, int widget_desc)
{
	...
	case WIDGET_SLIDER_VERT:
		...
		cm.found = 0;
		XCheckIfEvent(iniClient.dpy, &dumb, Widget_check_motion, (char *)&cm);
		if (cm.found > 0) {
			break;
		}
...
static void Widget_button(XEvent *event, int widget_desc, bool pressed)
{
	...
	case WIDGET_BUTTON_MENU:
		...
		if (pressed == true) {
			XMoveWindow(iniClient.dpy, pulldown_widget->window,
						event->xbutton.x_root - event->xbutton.x - 1,
						event->xbutton.y_root - event->xbutton.y
							+ widget->height);
			XMapRaised(iniClient.dpy, pulldown_widget->window);
			XFlush(iniClient.dpy);
		} else {
			XUnmapWindow(iniClient.dpy, pulldown_widget->window);
					   ...
					   int border)
{
	...
	window =
		XCreateSimpleWindow(iniClient.dpy, parent_window,
							x, y, width, height,
							border, colors[borderColor].pixel,
							colors[windowColor].pixel);
	XSelectInput(iniClient.dpy, window, 0);
						   ...
						   void *user_data)
{
	...
	window =
		XCreateSimpleWindow(iniClient.dpy, parent_widget->window,
							x, y, width, height,
							border, colors[borderColor].pixel,
							colors[buttonColor].pixel);
	XSelectInput(iniClient.dpy, window,
							  ...
							  void *user_data)
{
	...
	if (pulldown_desc == NO_WIDGET) {
		...
		window = XCreateWindow(iniClient.dpy,
	...
	window =
		XCreateSimpleWindow(iniClient.dpy, pulldown_widget->window,
					  ...
					  void *user_data)
{
	...
	window =
		XCreateSimpleWindow(iniClient.dpy, parent_widget->window,
							...
							border, colors[borderColor].pixel,
							colors[BLACK].pixel);
	XSelectInput(iniClient.dpy, window, ExposureMask);
...
{
	int 						disp_width = DisplayWidth(iniClient.dpy, 0),
								disp_height = DisplayHeight(iniClient.dpy, 0),
								x, y,
								popup_desc;
	widget_t					*widget;
	Window						window;
	...
	widget->name = "popup";
	XStoreName(iniClient.dpy, widget->window, window_name);
	XSetIconName(iniClient.dpy, widget->window, icon_name);
	XSetTransientForHint(iniClient.dpy, widget->window, top);
	return popup_desc;
}
...
int Widget_backing_store(int widget_desc, int mode)
{
	...
	XChangeWindowAttributes(iniClient.dpy, widget->window, CWBackingStore, &sattr);
	return widget_desc;
}
...
int Widget_set_background(int widget_desc, int bgcolor)
{
	...
	XChangeWindowAttributes(iniClient.dpy, widget->window, CWBackPixel, &sattr);
	return widget_desc;
}
...
int Widget_map_sub(int widget_desc)
{
	...
	if ((widget = Widget_pointer(widget_desc)) != NULL) {
		XMapSubwindows(iniClient.dpy, widget->window);
	} else {
		seterrno(0);
...
int Widget_map(int widget_desc)
{
	...
	if ((widget = Widget_pointer(widget_desc)) != NULL) {
		XMapWindow(iniClient.dpy, widget->window);
	} else {
		seterrno(0);
...
int Widget_unmap(int widget_desc)
{
	...
	if ((widget = Widget_pointer(widget_desc)) != NULL) {
		XUnmapWindow(iniClient.dpy, widget->window);
	} else {
		seterrno(0);
...
int Widget_raise(int widget_desc)
{
	...
	if ((widget = Widget_pointer(widget_desc)) != NULL) {
		XMapRaised(iniClient.dpy, widget->window);
	} else {
		seterrno(0);
...
static int Widget_viewer_calculate_view(int viewer_desc)
{
	...
	XResizeWindow(iniClient.dpy, widget->window, new_width, new_height);
	XMoveWindow(iniClient.dpy, widget->window, new_x, new_y);
	viewerw->visible_x = new_x;
	viewerw->visible_y = new_y;
						 ...
						 XFontStruct *font)
{
	...
	viewerw->font = font;
	viewerw->overlay =
		XCreateSimpleWindow(iniClient.dpy, popup_widget->window,
							0, 0, viewer_width, viewer_height,
							0, colors[borderColor].pixel,
							colors[windowColor].pixel);
	XSelectInput(iniClient.dpy, viewerw->overlay, 0);
	Widget_bit_gravity(viewerw->overlay, NorthWestGravity);
	window =
		XCreateSimpleWindow(iniClient.dpy, viewerw->overlay,
							0, 0, viewer_width, viewer_height,
							0, colors[borderColor].pixel,
							colors[windowColor].pixel);
	XSelectInput(iniClient.dpy, window, ExposureMask);
	...
	XMapWindow(iniClient.dpy, window);
...
int Widget_update_viewer(int popup_desc, const char *buf, int len)
{
	...
	last_visible = (viewer_sub->visible_y + viewer_widget->height - 20) 	/ text_height;
