/* $Id: talk.cpp,v 1.7 2004/04/21 06:43:40 dick Exp $
 *
...
static void Talk_create_window(void)
{
     ...
     */
    talk_w
	= XCreateSimpleWindow(iniClient.dpy, draw,
	...
	XSelectInput(iniClient.dpy, talk_w, KeyPressMask | KeyReleaseMask | ExposureMask);
    } else {
	XSelectInput(iniClient.dpy, talk_w, ButtonPressMask | ButtonReleaseMask |
...
void Talk_cursor(bool visible)
{
    ...
    if (visible == false) {
	XSetForeground(iniClient.dpy, talkGC, colors[BLACK].pixel);
	XDrawString(iniClient.dpy, talk_w, talkGC,
		    talk_cursor.offset + TALK_INSIDE_BORDER,
		    talkFont->ascent + TALK_INSIDE_BORDER,
		    "_", 1);
	XSetForeground(iniClient.dpy, talkGC, colors[WHITE].pixel);
	if (talk_cursor.point < (int)strlen(talk_str)) {
	    /* cursor _in message */
	    if ((iniClient.selectionAndHistory != 0)
		...
		XSetForeground(iniClient.dpy, talkGC, colors[DRAW_EMPHASIZED].pixel);
	    }
	    XDrawString(iniClient.dpy, talk_w, talkGC,
			talk_cursor.offset + TALK_INSIDE_BORDER,
			talkFont->ascent + TALK_INSIDE_BORDER,
...
void Talk_map_window(bool map)
{
	...
	XMapWindow(iniClient.dpy, talk_w);
	talk_mapped = true;

	XQueryPointer(iniClient.dpy, DefaultRootWindow(iniClient.dpy),
		      &root, &child, &root_x, &root_y, &win_x, &win_y,
		      &keys_buttons);
	XWarpPointer(iniClient.dpy, None, talk_w,
		     0, 0, 0, 0,
		     TALK_WINDOW_WIDTH - (TALK_WINDOW_HEIGHT / 2),
		     TALK_WINDOW_HEIGHT / 2);
	XFlush(iniClient.dpy);	/* warp pointer ASAP. */
    }
    else if (talk_created == true) {
	XUnmapWindow(iniClient.dpy, talk_w);
	XWarpPointer(iniClient.dpy, None, root, 0, 0, 0, 0, root_x, root_y);
	XFlush(iniClient.dpy);	/* warp pointer ASAP. */
	talk_mapped = false;
	/* reset browsing */
	history_pos = -1;
...
static void Talk_refresh(void)
{
    ...
    if (!iniClient.selectionAndHistory || !talk_mapped)
	return;
	...
	XClearWindow(iniClient.dpy, talk_w);
	return;
    }
...
static void Add_msg_to_history(char *message)
{
    ...
    if (!iniClient.selectionAndHistory || strlen(message) == 0) {
	return; /* unexpected. nothing to add */
    }

    msg_set = HistoryMsg;
    /* pipe the msgs through the buffer, the latest getting into [0] */
    tmp = msg_set[iniClient.maxLinesInHistory - 1];
    for (i = iniClient.maxLinesInHistory - 1; i > 0; i--) {
...
static char *Get_msg_from_history(int* pos, char *message, keys_t direction)
{
    ...
    if (direction == KEY_DUMMY && (*pos < 0 || *pos > iniClient.maxLinesInHistory-1)) {
	*pos = 0;
    }
	...
	if (direction == KEY_TALK_CURSOR_UP) {
            (*pos)++;
            if (*pos >= iniClient.maxLinesInHistory) {
                *pos = 0; /* wrap */
            }
	} else if (direction == KEY_TALK_CURSOR_DOWN) {
	    (*pos)--;
	    if (*pos < 0) {
		*pos = iniClient.maxLinesInHistory - 1; /*wrap*/
	    }
	}
...
void Print_messages_to_stdout(void)
{
    ...
    if (BIT(iniClient.instruments, SHOW_REVERSE_SCROLL)) {
	direction	= -1;
	offset		= iniClient.maxMessages - 1;
    } else {
	direction	= 1;
	offset		= 0;
    }
    ...
    for (k = 0; k < iniClient.maxMessages; k++) {
    ...
    for (k = iniClient.maxMessages - 1; k >= 0; k--) {
	i = direction * k + offset;
...
static void Talk_delete_emphasized_text(void)
{
    ...
    if (! (iniClient.selectionAndHistory && selection.talk.state == SEL_EMPHASIZED)) {
	return;
    }
...
int Talk_do_event(XEvent *event)
{
	...
	case '\n':
		...
		if (iniClient.selectionAndHistory && save_talk_str) {
	...
	XFlush(iniClient.dpy);	/* needed in case we don't get frames to draw soon. */
...
int Talk_paste(char *data, int data_len, bool overwrite)
{
	...
	if (char_width == 0)
		char_width = 1;
    if (!iniClient.selectionAndHistory || !data || data_len == 0 || strlen(data) == 0) {
	return 0;
    }
...
{
    if (talk_created) {
	XMoveResizeWindow(iniClient.dpy, talk_w,
			  TALK_WINDOW_X, TALK_WINDOW_Y,
			  TALK_WINDOW_WIDTH, TALK_WINDOW_HEIGHT);
    }
}
...
void Talk_window_cut(XButtonEvent* xbutton)
{
	 ...
	 */
	XSetSelectionOwner(iniClient.dpy, XA_PRIMARY, talk_w, CurrentTime);
	...
	XStoreBytes(iniClient.dpy, selection.txt, selection.len);
...
void Talk_cut_from_messages(XButtonEvent* xbutton)
{
    ...
    */
	 ...
	 */
	XSetSelectionOwner(iniClient.dpy, XA_PRIMARY, draw, CurrentTime);
	XStoreBytes(iniClient.dpy, selection.txt, selection.len);
