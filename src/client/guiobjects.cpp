/* $Id: guiobjects.cpp,v 1.13 2004/06/03 06:04:42 dick Exp $
 *
 ...
 *
 * Revision 1.6  2002/06/10 02:25:12  dick
 * Teamshots were using the wrong ini size
 *
 ...
 * Encap îlate all client resources into new class Ini.
 *
...
void Gui_paint_ball(int x, int y)
{
	if (!iniClient.blockBitmaps) {
		...
		if (BIT(iniClient.instruments, SHOW_TEXTURED_BALLS)) {
			if (ballTile == None) {
				ballTile = Texture_ball();
				if (ballTile == None) {
					CLR_BIT(iniClient.instruments, SHOW_TEXTURED_BALLS);
				}
			}
			if (ballTile != None) {
				XSetTile(iniClient.dpy, gc, ballTile);
				XSetFillStyle(iniClient.dpy, gc, FillTiled);
			}
		}
		else {
			ballTile = None;
		...
		y = Y(y);
		if (ballTile != None) {
			XSetTSOrigin(iniClient.dpy, gc, WINSCALE(x - BALL_RADIUS), WINSCALE(y - BALL_RADIUS));
		...
		if (ballTile != None) {
			XSetFillStyle(iniClient.dpy, gc, FillSolid);
		}
	}
	else {
...
///////////////////////////////////////////////////////////////////////////////
/* used by Paint_mine */
static void Gui_paint_mine_name(int x, int y, PCSTR name) 
{
...
void Gui_paint_mine(int x, int y, int teammine, PCSTR name)
{
	if (!iniClient.blockBitmaps) {
		...
		if (lastScaleFactor != iniClient.scaleFactor) {
			int 				i;
			lastScaleFactor = iniClient.scaleFactor;
			for (i = 1; i < 21; ++i) {
		...
		rd.drawLines(iniClient.dpy, p_draw, gc,
...
void Gui_paint_spark(int color, int x, int y)
{
	...
	Rectangle_add(color, 
				x - iniClient.sparkSize/2,
				y - iniClient.sparkSize/2,
				iniClient.sparkSize, iniClient.sparkSize);
}
...
void Gui_paint_wreck(int x, int y, bool deadly, int wtype, int rot, int size) 
{
	...
	rd.drawLines(iniClient.dpy, p_draw, gc, points, cnt, 0);
...
{
	if (!iniClient.blockBitmaps) {
		int z = iniClient.shotSize/2;

		if (iniClient.showNastyShots) {
				Gui_paint_nastyshot(color, x, y, z);
		} else {
				Rectangle_add(color,
						  x - z,
						  y - z,
						  iniClient.shotSize, iniClient.shotSize);
		}
	}
	else {
		int s_size = (iniClient.shotSize > 8) ? 8 : iniClient.shotSize ;
		int z = s_size / 2;
...
{
	if (!iniClient.blockBitmaps) {
		Gui_paint_nastyshot(color, x, y, iniClient.teamShotSize/2);
	}
	else {
		int s_size = (iniClient.teamShotSize > 8) ? 8 : iniClient.shotSize ;
		int z = s_size / 2;
...
void Gui_paint_missiles_begin(void)
{
	...
	SET_FG(colors[WHITE].pixel);
#endif
	XSetLineAttributes(iniClient.dpy, gc, 4,
...
{
	XSetLineAttributes(iniClient.dpy, gc, 0,
					   LineSolid, CapButt, JoinMiter);
}
...
{
	XSetLineAttributes(iniClient.dpy, gc, 3,
						   LineSolid, CapButt, JoinMiter);
}
...
void Gui_paint_paused(int x, int y, int count)
{
	if (!iniClient.blockBitmaps) {
		...
		if (count <= 0 || loops % 10 >= 5) {
			SET_FG(colors[iniClient.mono?BLACK:WHITE].pixel);
...
void Gui_paint_connector(int x0, int y0, int x1, int y1, int tractor)
{
	if (tractor) {
		...
		SET_FG(colors[WHITE].pixel);
#endif
		rd.setDashes(iniClient.dpy, gc, 0, cdashes, NUM_CDASHES);
	} else {
#ifdef _WINDOWS
		SET_FG(colors[WHITE].pixel+CLOAKCOLOROFS);		/* dashed line */
#endif
		rd.setDashes(iniClient.dpy, gc, 0, dashes, NUM_DASHES);
	}
	rd.drawLine(iniClient.dpy, p_draw, gc,
			  WINSCALE(X(x0)), WINSCALE(Y(y0)),
			  WINSCALE(X(x1)), WINSCALE(Y(y1)));
...
void Gui_paint_all_connectors_begin()
{
	...
	if (gcv.line_style != LineOnOffDash) {
		...
		mask |= GCDashOffset;
#endif
		XChangeGC(iniClient.dpy, gc, mask, &gcv);
	}
...
static void Gui_paint_marking_lights(int id, int x, int y, ShipObj* ship, int dir)
{
	...
	if (((loops + id) & 0xF) == 0) {
		...
		for (lcnt = 0; lcnt < ship->num_r_light; lcnt++) {
			int rightLightColor = iniClient.maxColors > 4 ? 4 : BLUE;
...
static void Gui_paint_ship_cloaked(int ship_color, XPoint *points, int point_count)
{
	...
	rd.drawLines(iniClient.dpy, p_draw, gc, points, point_count, 0);
					...
					int shield, int deflector, int eshield)
{
	 ...
	 * his/her ship.
	 */
	if (BIT(iniClient.instruments, SHOW_SHIP_NAME)
		&& self != NULL
		&& self->id != id
		&& other != NULL) {
	...
	if (cloak == 0 && phased == 0) {
		if (!iniClient.blockBitmaps || !blockBitmapShips) {
