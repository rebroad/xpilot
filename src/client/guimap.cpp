/* $Id: guimap.cpp,v 1.11 2004/06/03 06:04:42 dick Exp $
 *
 * guimap - drawing routines for the playfield.
 *
...
/*
 * $Log: guimap.cpp,v $
 * Revision 1.11  2004/06/03 06:04:42  dick
 ...
 *
 * Revision 1.9  2004/01/20 15:14:26  dick
 ...
 *
 * Revision 1.7  2002/09/03 07:57:46  dick
 ...
 *
 * Revision 1.4  2002/05/18 20:55:33  dick
...
void Gui_paint_cannon(int x, int y, int type)
{
	...
	points[3] = points[0];
	rd.drawLines(iniClient.dpy, p_draw, gc, points, 4, 0);
...
void Gui_paint_fuel(int x, int y, int fuel)
{
    ...
    if (!iniClient.blockBitmaps || !fuel_images) {
#define FUEL_BORDER 2
	...
	if (!text_width || lastScaleFactor != iniClient.scaleFactor)
	{
	...
	rd.drawString(iniClient.dpy, p_draw, gc, x, y, s, 1);
...
void Gui_paint_base(int x, int y, int xi, int yi, int type)
{
	...
	if (size) {
	    rd.drawString(iniClient.dpy, p_draw, gc, x, y, s, other ? 2 : 1);
	...
	if (other) {
	     rd.drawString(iniClient.dpy, p_draw, gc, x, y,
	...
	if (other) {
	    rd.drawString(iniClient.dpy, p_draw, gc, x, y, other->name, other->name_len);
	    Erase_rectangle(x - 1,
			    y - gameFont->ascent - 1,
			    other->name_width + 2,
			    gameFont->ascent + gameFont->descent + 2);
...
void Gui_paint_decor(int x, int y, int xi, int yi, int type, bool last, bool more_y)
{
    ...
    if (BIT(iniClient.instruments, SHOW_TEXTURED_DECOR)) {
	if (!decorTileReady) {
	...
	if (decorTileReady == 1) {
	    decorTileDoit = true;
	    XSetTile(iniClient.dpy, gc, decorTile);
	    XSetTSOrigin(iniClient.dpy, gc, -WINSCALE(realWorld.x), WINSCALE(realWorld.y));
	...
	if (mask & DECOR_LEFT) {
		...
		: !(decor[Setup->map_data[(xi - 1) * Setup->y + yi]]
		    & DECOR_RIGHT)) {
		Segment_add(iniClient.decorColor,
			    X(x),
			    Y(y),
			    X(x),
			    Y(y+BLOCK_SZ));
	...
	if (mask & DECOR_RIGHT) {
	    if (!BIT(iniClient.instruments, SHOW_OUTLINE_DECOR)
		|| ((xi == Setup->x - 1)
		    ? (!BIT(Setup->mode, WRAP_PLAY)
...
{
    Rectangle_add(BLUE, X(x + BLOCK_SZ / 2) - (iniClient.mapPointSize >> 1),
		  Y(y + BLOCK_SZ / 2) - (iniClient.mapPointSize >> 1),
		  size, size);
}
