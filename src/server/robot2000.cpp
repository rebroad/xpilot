...
//typedef	robot_default_data_t	robot2000_data_t;
 ...
 */
static void Robot2000_round_tick(void);
static void Robot2000_create(int ind, char *str);
static void Robot2000_go_home(int ind);
static void Robot2000_play(int ind);
static void Robot2000_set_war(int ind, int victim_id);
static int Robot2000_war_on_player(int ind);
static void Robot2000_message(int ind, const char *str);
static void Robot2000_destroy(int ind);
       int Robot2000_setup(robot_type_t *type_ptr);
 ...
 */
static robot_type_t robot2000_type = {
    "robot2000",
    Robot2000_round_tick,
    Robot2000_create,
    Robot2000_go_home,
    Robot2000_play,
    Robot2000_set_war,
    Robot2000_war_on_player,
    Robot2000_message,
    Robot2000_destroy
};
 ...
 */
static robot2000_data_t *Robot2000_get_data(player *pl)
{
    return (robot2000_data_t *)pl->robot_data_ptr->private_data;
}
...
static void Robot2000_create(int ind, char *str)
{
    ...
    robot2000_data_t	*my_data;

    if (!(my_data = (robot2000_data_t *)malloc(sizeof(*my_data)))) {
	error("no mem for default robot");
	theWorld.EndGame();
	return;
    }
    strlcpy(pl->realname, "robot2000", MAX_CHARS);
...
{
    player			*pl = Players[ind];
    robot2000_data_t	*my_data = Robot2000_get_data(pl);
 ...
 */
static void Robot2000_message(int ind, const char *message)
{
#if 0
    player			*pl = Players[ind];
    robot2000_data_t	*my_data = Robot2000_get_data(pl);
...
static int Robot2000_play_check_map(int ind)
{
    player			*pl = Players[ind];
    int				j;
...
static void Robot2000_play_check_objects(int ind,
...
static void Robot2000_play_check_lasers(int ind)
{
    player			*pl = Players[ind];
    int				j;
    int				dx, dy;
...
static void Robot2000_play(int ind)
{
    ...
    Robot2000_play_check_objects(ind,
				     &item_i, &item_dist, &item_imp,
				     &mine_i, &mine_dist);

    Robot2000_play_check_lasers(ind);
    ...
    if (Robot2000_play_check_map(ind) == 1) {
	return;
    }
