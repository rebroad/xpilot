 ...
 *
 *  Revision 1.18  2004/02/23 02:06:09  dick
 *  robot.h becomes Robot.h
 *
 *  Revision 1.17  2004/02/20 06:27:47  dick
 *  SendPlayMessage becomes BroadcastPlayMessage() because it sends a play message
 *  to all connections.
 *
 *  Revision 1.16  2004/02/05 14:56:58  cppc
 *  First cut at integrating 'it' play mode.
 *
 *  Revision 1.15  2004/02/03 03:39:54  dick
 *  Add Windows precompiled header support.
 *
 *  Revision 1.14  2002/09/16 21:47:07  dick
 *  Fix bug in targetDeadTime, and let it be a float (why not have 20.5 seconds)
 *
 ...
 *
 *  Revision 1.11  2002/08/01 14:32:10  dick
 ...
 *
 *  Revision 1.9  2002/07/07 04:37:52  dick
 *  remove theWorld refs
 *
 *  Revision 1.8  2002/06/30 07:00:44  dick
...
void World::WalldistAlloc(void)
{
	...
	wall_ptr = walldist;
	wall_line = (unsigned char *)(wall_ptr + blockWidth);
	for (x = 0; x < blockWidth; x++) {
		*wall_ptr = wall_line;
		wall_ptr += 1;
		wall_line += blockHeight;
	}
}
...
void World::WalldistInit(void)
{
	int 				x, y, dx, dy, wx, wy;
	int 				dist;
	int 				mindist;
	int 				maxdist = 2 * MIN(blockWidth, blockHeight);
	int 				newdist;
	...
	q = (Qelmt_t *)malloc(blockWidth * blockHeight * sizeof(Qelmt_t));
	if (!q) {
	...
	for (x = 0; x < blockWidth; x++) {
		for (y = 0; y < blockHeight; y++) {
			if (BIT((1 << block[x][y]), WALLDIST_MASK)
				&& (block[x][y] != WORMHOLE
					|| wormHoles[WormXY(x, y)].type != WORM_OUT)) {
				walldist[x][y] = 0;
				q[qback].x = x;
				q[qback].y = y;
				qback++;
	...
	if (!BIT(rules->mode, WRAP_PLAY)) {
		for (x = 0; x < blockWidth; x++) {
			for (y = 0; y < blockHeight; y += (!x || x == blockWidth - 1)
										? 1 : (blockHeight - (blockHeight > 1))) {
				if (walldist[x][y] > 1) {
					walldist[x][y] = 2;
					q[qback].x = x;
					q[qback].y = y;
	...
	while (qfront != qback) {
		x = q[qfront].x;
		y = q[qfront].y;
		if (++qfront == blockWidth * blockHeight) {
			qfront = 0;
		}
		dist = walldist[x][y];
		mindist = dist + 2;
		...
		for (dx = -1; dx <= 1; dx++) {
			if (BIT(rules->mode, WRAP_PLAY)
				|| (x + dx >= 0 && x + dx < blockWidth)) {
				wx = WRAP_XBLOCK(this, x + dx);
				for (dy = -1; dy <= 1; dy++) {
					if (BIT(rules->mode, WRAP_PLAY)
						|| (y + dy >= 0 && y + dy < blockHeight)) {
						wy = WRAP_YBLOCK(this, y + dy);
						if (walldist[wx][wy] > mindist) {
							...
							if (dist == 0) {
								...
								else if (block[x][y] == REC_RD) {
									if (dx == -1 && dy == +1) {
										newdist = mindist + 1;
									}
								}
								else if (block[x][y] == REC_LU) {
									if (dx == +1 && dy == -1) {
										newdist = mindist + 1;
									}
								}
								else if (block[x][y] == REC_RU) {
									if (dx == -1 && dy == -1) {
										newdist = mindist + 1;
									}
								}
							...
							if (newdist < walldist[wx][wy]) {
								...
								q[qback].x = wx;
								q[qback].y = wy;
								if (++qback == blockWidth * blockHeight) {
									qback = 0;
								}
							}
						}
...
void World::WallsInit()
{
	WalldistAlloc();
	WalldistInit();
}

void World::TreasureInit()
{
	int i;
...
void World::MoveInit()
{
	mp.click_width = PIXEL_TO_CLICK(width);
	mp.click_height = PIXEL_TO_CLICK(height);

	options.maxObjectWallBounceSpeed->Limit(0, hypotenuse);
	options.maxShieldedWallBounceSpeed->Limit(0, hypotenuse);
	options.maxUnshieldedWallBounceSpeed->Limit(0, hypotenuse);
	options.maxShieldedPlayerWallBounceAngle->Limit(0, 180);
	options.maxUnshieldedPlayerWallBounceAngle->Limit(0, 180);
	options.playerWallBounceBrakeFactor->Limit(0, 1);
	options.objectWallBounceBrakeFactor->Limit(0, 1);
	options.objectWallBounceLifeFactor->Limit(0, 1);
	options.wallBounceFuelDrainMult->Limit(0, 1000);
	wallBounceExplosionMult = sqrt(options.wallBounceFuelDrainMult->GetDouble());

	mp.max_shielded_angle = (int)(options.maxShieldedPlayerWallBounceAngle->GetDouble() * RES / 360);
	mp.max_unshielded_angle = (int)(options.maxUnshieldedPlayerWallBounceAngle->GetDouble() * RES / 360);

	mp.obj_bounce_mask = 0;
	if (options.sparksWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_SPARK);
	}
	if (options.debrisWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_DEBRIS);
	}
	if (options.shotsWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_SHOT|OBJ_CANNON_SHOT);
	}
	if (options.itemsWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_ITEM);
	}
	if (options.missilesWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_SMART_SHOT|OBJ_TORPEDO|OBJ_HEAT_SHOT);
	}
	if (options.minesWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_MINE);
	}
	if (options.ballsWallBounce->GetBool()) {
		SET_BIT(mp.obj_bounce_mask, OBJ_BALL);
	}
	if (options.asteroidsWallBounce->GetBool()) {
 ...
 */
void World::MoveSegment(move_state_t *ms)
{
	int 				i;
	...
	block_type = this->block[block.x][block.y];
	...
	case WORMHOLE:
		...
		if (wormHoles[hole].type == WORM_OUT) {
			break;
		}
		if (mi->pl) {
			...
			if (BIT(mi->pl->status, WARPED)) {
				if (this->block[blk2.x][blk2.y] == WORMHOLE) {
					int oldhole = WormXY(blk2.x, blk2.y);
					if (wormHoles[oldhole].type == WORM_NORMAL
			...
			if (last >= 0
				...
				ms->done.x += (wormHoles[last].pos.x
					- wormHoles[hole].pos.x) * BLOCK_CLICKS;
				ms->done.y += (wormHoles[last].pos.y
					- wormHoles[hole].pos.y) * BLOCK_CLICKS;
				break;
			}
		}
		break;
	...
	case CANNON:
		...
		for (i = 0; ; i++) {
			if (cannons[i].blk_pos.x == block.x
				&& cannons[i].blk_pos.y == block.y) {
				break;
			}
		}
		ms->cannon = i;

		if (BIT(cannons[i].used, HAS_PHASING_DEVICE)) {
				break;
		}
	...
	case TREASURE:
		if (block_type == TREASURE) {
			if (mi->treasure_crashes) {
				...
				for (i = 0; ; i++) {
					if (treasures[i].pos.x == block.x
						&& treasures[i].pos.y == block.y) {
						break;
					}
				}
				ms->treasure = i;
				...
				if (ms->treasure == ball->treasure) {
					 ...
					 */
					Player		*pl = NULL;
					Treasure	*tt = &treasures[ms->treasure];
				...
				if (BIT(rules->mode, TEAM_PLAY)
					...
					sprintf(msg," < The ball was loose for %ld frames >",
							LONG_MAX - ball->life);
					BroadcastPlayMessage(msg);
					if (options.captureTheFlag->GetBool()
						&& !treasures[ms->treasure].have
						&& !treasures[ms->treasure].empty) {
						strcpy(msg, "Your treasure must be safe before you can cash an opponent's!");
						players[getInd[ball->owner]]->SetMessage(msg);
					} else if (PunishTeam(players[getInd[ball->owner]],
	...
	case TARGET:
		if (block_type == TARGET) {
			if (mi->target_crashes) {
				 ...
				 * ms->target = i;
				 */
				ms->target = i = itemID[block.x][block.y];

				if (!options.targetTeamCollision->GetBool()) {
					int team;
					if (mi->pl) {
						team = mi->pl->team;
					}
	...
	if (wall_bounce) {
		...
		for (bit = 1; bit <= wall_bounce; bit <<= 1) {
			...
			case BounceHorLo:
				...
				blk2.y = block.y;
				if (BIT(1 << this->block[blk2.x][blk2.y],
...
void World::CannonDies(move_state_t *ms)
{
		Cannon* 		cannon = cannons + ms->cannon;
		int 					x = (int)cannon->pix_pos.x;
		int 					y = (int)cannon->pix_pos.y;
		int 					killer = -1;
		Player* 		pl = NULL;

		cannon->dead_time = options.cannonDeadTime->GetSec();
		cannon->conn_mask = 0;
		block[cannon->blk_pos.x][cannon->blk_pos.y] = SPACE;
		cannon->ThrowItems();
		cannon->Init();
		SoundPlaySensors(this, x, y, CANNON_EXPLOSION_SOUND);
		MakeDebris(
		/* pos.x, pos.y   */ x, y,
		/* vel.x, vel.y   */ 0.0, 0.0,
		/* owner id 	  */ NO_ID,
		...
		if (pl) {
				...
				ScoreServerScoreEvent(pl, sc, s_brCannon, PlayerNone, 0, ScoreShot);
		}
}

void World::ObjectHitsTarget(move_state_t *ms, long player_cost)
{
	Target				*targ = &targets[ms->target];
	...
	case OBJ_SHOT:
				if (options.shotHitFuelDrainUsesKineticEnergy->GetBool()) {
						DFLOAT sspeed = options.shotSpeed->GetDouble();
						DFLOAT smass  = options.shotMass->GetDouble();
	...
	targ->dead_time = (int)(options.targetDeadTime->GetDouble()*GetFPS());
	...
	if (somebody_flag) {
		for (j = 0; j < numTargets; j++) {
			if (targets[j].team == targ->team) {
				targets_total++;
				if (targets[j].dead_time == 0) {
					targets_remaining++;
				}
			}
		}
	...
	SoundPlaySensors(this, x, y, DESTROY_TARGET_SOUND);
	...
	if (targets_remaining > 0) {
		 ...
		 */
		if (options.targetTeamCollision->GetBool() && targets_total < 10) {
	...
	if (options.targetKillTeam->GetBool()) {
	...
	for (j = 0; j < numPlayers; j++) {
		...
		if (players[j]->team == targ->team) {
			...
			SCORE(players[j], -sc, targ->pos.x, targ->pos.y,
				  "Target: ");
		}
		else if (players[j]->team == players[killer]->team &&
				 (players[j]->team != TEAM_NOT_SET || j == killer)) {
			SCORE(players[j], por, targ->pos.x, targ->pos.y,
				  "Target: ");
		}
	}
}

void World::ObjectCrash(move_state_t *ms)
{
	Object				*obj = ms->mip->obj;
...
*/
		if (!BIT(obj->type, OBJ_SPARK | OBJ_DEBRIS)) {
			MakeDebris(CLICK_TO_FLOAT(ms->pos.x),
	...
	case CrashCannon:
		...
		if (BIT(obj->type, OBJ_ITEM)) {
			cannons[ms->cannon].AddItem(obj->info, obj->count);
		} else {
				if (!BIT(cannons[ms->cannon].used, HAS_EMERGENCY_SHIELD)) {
						if (cannons[ms->cannon].items[ITEM_ARMOR] > 0)
								cannons[ms->cannon].items[ITEM_ARMOR]--;
						else
								CannonDies(ms);
				}
		}
...
void World::MoveObject(Object *obj)
{
	int 				nothing_done = 0;
	int 				dist;
	move_info_t 		mi;
	...
	if (dist > 2) {
		...
		if (sqr(max) >= sqr(obj->vel.x) + sqr(obj->vel.y)) {
			...
			x = WRAP_XCLICK(this, x);
			y = WRAP_YCLICK(this, y);
	...
	mi.pl = NULL;
	mi.obj = obj;
	mi.edge_wrap = BIT(rules->mode, WRAP_PLAY);
	mi.edge_bounce = options.edgeBounce->GetBool();
	...
	for (;;) {
		MoveSegment(&ms);
		if (!(ms.done.x | ms.done.y) != 0) {
			pos_update |= ((ms.crash | ms.bounce) != 0);
			if (ms.crash) {
				break;
			}
			if (ms.bounce && ms.bounce != BounceEdge) {
				if (obj->type != OBJ_BALL)
					obj->life = (long)(obj->life * options.objectWallBounceLifeFactor->GetDouble());
				if (obj->life <= 0) {
					break;
				}
				/*
				...
				if (sqr(ms.vel.x) + sqr(ms.vel.y) > sqr(options.maxObjectWallBounceSpeed->GetDouble())) {
					obj->life = 0;
					break;
				}
				ms.vel.x *= options.objectWallBounceBrakeFactor->GetDouble();
				ms.vel.y *= options.objectWallBounceBrakeFactor->GetDouble();
				ms.todo.x = (int)(ms.todo.x * options.objectWallBounceBrakeFactor->GetDouble());
				ms.todo.y = (int)(ms.todo.y * options.objectWallBounceBrakeFactor->GetDouble());
			}
			if (++nothing_done >= 5) {
...
void World::PlayerCrash(move_state_t *ms, int pt, bool turning)
{
		Player* 		pl = ms->mip->pl;
		int 					ind = getInd[pl->id];
		PCSTR			howfmt = NULL;
		PCSTR			hudmsg = NULL;
		ScoreType		st = ScoreNone;
		...
		case CrashWall:
				...
				hudmsg = s_brWall;
				st = ScoreCrash;
				SoundPlaySensors(this, pl->pos.x, pl->pos.y, PLAYER_HIT_WALL_SOUND);
				break;
		...
		case CrashCannon:
						...
						!= (HAS_SHIELD|HAS_EMERGENCY_SHIELD)) 
				{
						...
						hudmsg = s_brCannon;
						st = ScoreCrash;
						SoundPlaySensors(this, pl->pos.x, pl->pos.y, PLAYER_HIT_CANNON_SOUND);
				}
				if (!BIT(cannons[ms->cannon].used, HAS_EMERGENCY_SHIELD)) {
						CannonDies(ms);
				}
		...
		if (howfmt && hudmsg) {
				...
				if (num_pushers == 0) {
						...
						SCORE(pl, -sc,
								...
								hudmsg);
						if (st != ScoreNone)
								ScoreServerScoreEvent(hudmsg, PlayerNone, 0, pl, -sc, st);
						strcat(msg, ".");
						BroadcastPlayMessage(msg);
				}
				else {
						...
						DFLOAT	average_pusher_score = total_pusher_score
										/ total_pusher_count;
						...
						for (i = 0; i < num_pushers; i++) {
								...
								sc = cnt[i] * Rate(pusher->score, pl->score)
										* options.shoveKillScoreMult->GetDouble()
										/ total_pusher_count;
								...
								if (options.tagGame->GetBool())
								{
									if (tag == pusher->id)
									{
										sc *= options.tagItKillMult->GetDouble();
										pusher_is_tagged = 1;
									}
									else if (was_tagged && num_pushers == 1)
									{
										sc *= options.tagKillItMult->GetDouble();
										TransferTag(ind, getInd[pusher->id]);
									}
								}
						...
						if (options.tagGame->GetBool() && num_pushers == 1)
						{
							if (was_tagged)
								sc *= options.tagKillItMult->GetDouble();
...
void World::MovePlayer(int ind)
{
	...
	if (BIT(pl->status, PLAYING|PAUSE|GAME_OVER|KILLED) != PLAYING) {
		if (!BIT(pl->status, KILLED|PAUSE)) {
			...
			pos.x = WRAP_XCLICK(this, pos.x);
			pos.y = WRAP_YCLICK(this, pos.y);
...
/* Figure out which friction to use. */
		fric = options.friction->GetDouble();
		if (BIT(pl->used, HAS_PHASING_DEVICE)) {
				switch (block[pl->pos.bx][pl->pos.by]) {
				case FRICTION:
						fric = options.blockFriction->GetDouble();
						break;
				default:
						break;
				}
	...
	cor_res = MOD2(options.coriolis->GetInt() * RES / 360, RES);
	oldvx = pl->vel.x;
	oldvy = pl->vel.y;
	...
	for (;; moves_made++) {
		...
		pos.x = WRAP_XCLICK(this, pos.x);
		pos.y = WRAP_YCLICK(this, pos.y);
		iblock.x = pos.x / BLOCK_CLICKS;
		iblock.y = pos.y / BLOCK_CLICKS;

		if (walldist[iblock.x][iblock.y] > 3) {
			int maxcl = ((walldist[iblock.x][iblock.y] - 3) * BLOCK_CLICKS) >> 1;
			todo = ms[0].todo;
			sign.x = (todo.x < 0) ? -1 : 1;
			sign.y = (todo.y < 0) ? -1 : 1;
		...
		for (i = 0; i < pl->ship->num_points; i++) {
			MoveSegment(&ms[i]);
			pos_update |= ((ms[i].crash | ms[i].bounce) != 0);
			if (ms[i].crash) {
				crash = i;
				break;
			}
		...
		if (crash != -1) {
			...
			if (ms[worst].bounce != BounceEdge) {
				...
				DFLOAT	b = 1 - 0.5f * options.playerWallBounceBrakeFactor->GetDouble();
				long	cost = (long) (b * m * v);
				int 	delta_dir,
						abs_delta_dir,
						wall_dir;
				double	max_speed = BIT(pl->used, HAS_SHIELD)
									? options.maxShieldedWallBounceSpeed->GetDouble()
									: options.maxUnshieldedWallBounceSpeed->GetDouble();
				...
				ms[worst].vel.x *= options.playerWallBounceBrakeFactor->GetDouble();
				ms[worst].vel.y *= options.playerWallBounceBrakeFactor->GetDouble();
				ms[worst].todo.x = (int)(ms[worst].todo.x * options.playerWallBounceBrakeFactor->GetDouble());
				ms[worst].todo.y = (int)(ms[worst].todo.y * options.playerWallBounceBrakeFactor->GetDouble());

				/* only use armor if neccessary */
				if (speed > max_speed
						&& max_speed < options.maxShieldedWallBounceSpeed->GetDouble()
				...
				if (abs_delta_dir > max_angle
						&& max_angle < options.maxShieldedWallBounceSpeed->GetDouble()
						&& !BIT(pl->used, HAS_SHIELD)
						&& BIT(pl->have, HAS_ARMOR)) {
						max_speed = options.maxShieldedWallBounceSpeed->GetDouble();
						max_angle = mp.max_shielded_angle;
				...
				if (abs_delta_dir <= RES/16) {
					pl->float_dir += (1.0f - options.playerWallBounceBrakeFactor->GetDouble()) * delta_dir;
					if (pl->float_dir >= RES) {
						pl->float_dir -= RES;
					}
				...
				if (BIT(pl->used, (HAS_SHIELD|HAS_EMERGENCY_SHIELD))
					...
					Add_fuel(&pl->fuel, (long)(-((cost << FUEL_SCALE_BITS)
										  * options.wallBounceFuelDrainMult->GetDouble())));
					Item_damage(pl, options.wallBounceDestroyItemProb->GetDouble());
				}
				if (!pl->fuel.sum && options.wallBounceFuelDrainMult->GetDouble() != 0) {
					crash = worst;
					ms[worst].crash = (ms[worst].target >= 0 ? CrashTarget :
									   CrashWallNoFuel);
					break;
				...
				if (cost) {
					...
					SoundPlaySensors(this, pl->pos.x, pl->pos.y, PLAYER_BOUNCED_SOUND);
					if (ms[worst].target >= 0) {
						cost <<= FUEL_SCALE_BITS;
						cost = (long)(cost * (options.wallBounceFuelDrainMult->GetDouble() / 4.0));
						ObjectHitsTarget(&ms[worst], cost);
					}
				}
			}
		}
	...
	if (ms[worst].crash) {
		PlayerCrash(&ms[worst], worst, false);
	}
	if (pos_update) {
		Player_position_remember(pl);
	}
...
void World::TurnPlayer(int ind)
{
	...
	if (crash != -1) {
		PlayerCrash(&ms[crash], crash, true);
	}
