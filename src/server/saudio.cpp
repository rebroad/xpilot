/* $Id: saudio.cpp,v 1.9 2004/05/07 01:15:25 dick Exp $
 *
...
/*
 * $Log: saudio.cpp,v $
 * Revision 1.9  2004/05/07 01:15:25  dick
 * Turn off audio debug
 *
 * Revision 1.8  2004/05/04 21:08:21  dick
 * Display Debug audio events
 *
 * Revision 1.7  2004/04/29 16:08:27  dick
 * compile with SOUND enabled.
 *
 * Revision 1.6  2004/02/18 01:22:14  dick
...
// Display/log sound events as they get sent.
#if 0 && defined(_DEBUG)
#define	DSOUND(_x)	_x
...
static void queue_audio(Player* pl, int index, int volume)
{
	...
	DSOUND(xpprintf("Sound: %d %d %s\n", index, volume, soundNames[index]);)
	if (prev)
		prev->next = a;
	else
		pl->audio = (void *) a;
...
void SoundPlayerOnOff(Player* pl, int onoff)
{
	...
	if (onoff) {
		if (!BIT(pl->status, WANT_AUDIO)) {
			SET_BIT(pl->status, WANT_AUDIO);
			SoundPlayPlayer(pl, START_SOUND);
		}
	}
	else {
 ...
 */
void SoundPlayPlayer(Player* pl, int index)
{
	SDBG(printf("SoundPlayPlayer %p, %d\n", pl, index);)
 ...
 */
void SoundPlayAll(World* w, int index)
{
	int i;
	...
	for (i = 0; i < w->numPlayers; i++) {
		if (BIT(w->players[i]->status, WANT_AUDIO)) {
			SoundPlayPlayer(w->players[i], index);
		}
	}
}
 ...
 */
void SoundPlaySensors(World* w, DFLOAT x, DFLOAT y, int index)
{
	int 			i,
					volume;
	DFLOAT			 dx,
...
void SoundPlayQueued(Player* pl)
{
	...
	while (p) {
		n = p->next;
		pl->conn->SendAudio(p->index, p->volume);
		free(p);
		p = n;
	}
}
