//
// "$Id: XPFileChooser2.cpp,v 1.7 2004/05/30 15:46:18 dick Exp $"
//
// More XPFileChooser routines.
//
...
// Contents:
//
//   XPFileChooser::count()             - Return the number of selected files.
//   XPFileChooser::directory()         - Set the directory in the file chooser.
//   XPFileChooser::filter()            - Set the filter(s) for the chooser.
//   XPFileChooser::newdir()            - Make a new directory.
//   XPFileChooser::value()             - Return a selected filename.
//   XPFileChooser::rescan()            - Rescan the current directory.
//   XPFileChooser::favoritesButtonCB() - Handle favorites selections.
//   XPFileChooser::fileListCB()        - Handle clicks (and double-clicks)
//                                          in the Fl_File_Browser.
//   XPFileChooser::fileNameCB()        - Handle text entry in the FileBrowser.
//   XPFileChooser::showChoiceCB()      - Handle show selections.
//   XPFileChooser::wildMap(int e)      - enable/disable wildMap mode.
//   XPFileChooser::wildNumberCB()      - callback when text number changed.
//   XPFileChooser::wildRandomCB()      - callback when random button pressed.
//   XPFileChooser::wildSliderCB()      - callback when slider moved.
...
/* Dick writes:
 * fltk recommends that you "copy the file chooser and change what you want"
 * in order to extend the chooser.  This is kinda lame, but since i want to
 * add a "Wild" button, here it is.
 *
 * WARNING!
 * Note that when playing with the file chooser, you should not modify
 * XPFileChooser.cpp and XPFileChooser.h .  These files are regenerated from
 * fluid.  Rather, you can change XPFileChooser.fl and XPFileChooser2.cpp .
 *
 */
/*
 * $Log: XPFileChooser2.cpp,v $
 * Revision 1.7  2004/05/30 15:46:18  dick
 * Use randommt() instead of rand()
 *
 * Revision 1.6  2004/02/21 06:53:18  dick
 * Don't leak the ServerOptions after we build the preview image
 *
 * Revision 1.5  2004/02/03 03:39:53  dick
 * Add Windows precompiled header support.
 *
 * Revision 1.4  2004/01/21 19:59:36  dick
 * Fix the .h changes by correctly modifying the .fl file and regenerating
 * the .cpp and .h  (These two are poorly named semi-temp/semi-permanent files.)
 *
 * Revision 1.3  2004/01/19 22:12:02  dick
 * Add a Random button and seed slider to assist in selecting WildMaps.
 * Add a text label to describe map attrs (currently only WxH).
 *
 * Revision 1.2  2004/01/19 07:37:07  dick
 * Linux fixes
 *
 * Revision 1.1  2004/01/19 06:46:16  dick
 * fltk recommends that you "copy the file chooser and change what you want"
 * in order to extend the chooser.  This is kinda lame, but since i want to
 * add a "Wild" button, here it is.  Oh, and we preview the map as a graphic.
 *
 */
#include "StdAfx.h"
...
Fl_Preferences	XPFileChooser::prefs_(Fl_Preferences::USER, "fltk.org", "filechooser");

const char	*XPFileChooser::add_favorites_label = "Add to Favorites";
const char	*XPFileChooser::all_files_label = "All Files (*)";
const char	*XPFileChooser::custom_filter_label = "Custom Filter";
const char	*XPFileChooser::existing_file_label = "Please choose an existing file!";
const char	*XPFileChooser::favorites_label = "Favorites";
const char	*XPFileChooser::filename_label = "Filename:";
#ifdef WIN32
const char	*XPFileChooser::filesystems_label = "My Computer";
#else
const char	*XPFileChooser::filesystems_label = "File Systems";
#endif // WIN32
const char	*XPFileChooser::manage_favorites_label = "Manage Favorites";
const char	*XPFileChooser::new_directory_label = "New Directory?";
const char	*XPFileChooser::preview_label = "Preview";
const char	*XPFileChooser::wildMap_label = "Wild Map";
const char	*XPFileChooser::show_label = "Show:";
Fl_File_Sort_F	*XPFileChooser::sort = fl_numericsort;
...
//    printf("XPFileChooser::count(): filename=\"%s\"\n", filename);
...
void
XPFileChooser::directory(const char *d)// I - Directory to change to
{
  char	*dirptr;			// Pointer into directory


//  printf("XPFileChooser::directory(\"%s\")\n", d == NULL ? "(null)" : d);
...
void
XPFileChooser::favoritesButtonCB()
{
...
//
// 'XPFileChooser::favoritesCB()' - Handle favorites dialog.
//

void
XPFileChooser::favoritesCB(Fl_Widget *w)
					// I - Widget
{
  int		i;			// Looping var
...
//
// 'XPFileChooser::fileListCB()' - Handle clicks (and double-clicks) in the
//                                   Fl_File_Browser.
//

void
XPFileChooser::fileListCB()
{
...
void
XPFileChooser::filter(const char *p)		// I - Pattern(s)
{
...
//
// 'XPFileChooser::preview()' - Enable or disable the preview tile.
//

void
XPFileChooser::preview(int e)// I - 1 = enable preview, 0 = disable preview
{
  previewButton->value(e);
  prefs_.set("preview", e);

  if (e) {
    int w = previewBox->h() * 4 / 3;
...
//
// 'XPFileChooser::previewCB()' - Timeout handler for the preview box.
//

void
XPFileChooser::previewCB(XPFileChooser *fc) {	// I - File chooser
  fc->update_preview();
}
...
//
// 'XPFileChooser::update_favorites()' - Update the favorites menu.
//

void
XPFileChooser::update_favorites()
{
  int		i;			// Looping var
  char		pathname[1024],		// Pathname
		menuname[2048];		// Menu name
...
//
// 'XPFileChooser::update_preview()' - Update the preview box...
//

void XPFileChooser::update_preview()
{
	const char		*filename;	// Current filename
	Fl_Image*	image = NULL;		// New image
	Fl_Image*	oldimage;		// Old image
	...
	else
	{
		...
		if (sos->LoadOptions(filename, OPT_MAP, true))
		{
			image = ServerOptionsToImage(sos);
			infoString.printf("%dx%d", sos->mapWidth->GetInt(), sos->mapHeight->GetInt());
		}

		mapInfo->label(infoString);
		if (image) {
...
XPFileChooser::value(int f)	// I - File number
{
  int		i;		// Looping var
  int		fcount;		// Number of selected files
  ...
  static char	pathname[1024];	// Filename + directory
	...
	if (wildMapButton->value())
	{
		i = (int)wildNumber->value();
		sprintf(pathname, "wild %d", i);
		return(pathname);
	}
...
//
// 'XPFileChooser::value()' - Set the current filename.
//

void
XPFileChooser::value(const char *filename)	// I - Filename + directory
{
  int	i,					// Looping var
...
/////////////////////////////////////////////////////////////////////////////////
int XPFileChooser::wildMap(int e)
{
	if (e)		// enabling wild map
	{
		fileList->deactivate();
		showChoice->deactivate();
		favoritesButton->deactivate();
		newButton->deactivate();
		wildLabel->show();
		wildNumber->show();
		wildSlider->show();
		wildRandomButton->show();
		fileNameLabel->hide();
		fileName->hide();
	}
	else
	{
		fileList->activate();
		showChoice->activate();
		favoritesButton->activate();
		newButton->activate();
		wildLabel->hide();
		wildNumber->hide();
		wildSlider->hide();
		wildRandomButton->hide();
		fileNameLabel->show();
		fileName->show();
...
void XPFileChooser::wildNumberCB()
{
	wildSlider->value(wildNumber->value());
	update_preview();
}

void XPFileChooser::wildRandomCB()
{
	int	r = randomMT();
	wildSlider->value(r);
	wildNumber->value(r);
...
void XPFileChooser::wildSliderCB()
{
	wildNumber->value(wildSlider->value());
	update_preview();
}


//
// End of "$Id: XPFileChooser2.cpp,v 1.7 2004/05/30 15:46:18 dick Exp $".
//
