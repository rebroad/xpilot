/* $Id: Sound.cpp,v 1.2 2007/02/17 20:19:45 dick Exp $
 *
...
#	ifdef _WINDOWS
		...
		alutLoadWAVFile(path.s, &format, &data, &size, &freq, &looping);
		if((err = alGetError()) != AL_NO_ERROR
			|| !data
			|| !size) {
...
#	ifdef _UNIX
		...
		err = alutLoadWAV(path.s, &data, &format, &size, &bits, &freq);
		if(err == AL_FALSE) {
