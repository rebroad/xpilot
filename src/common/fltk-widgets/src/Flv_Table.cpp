...
//static int drag_col=0, drag_row=-4, anchor_left, anchor_top;
...
//	Maybe for internal editors
	if (veditor && vediting)
	{
		int x, y, w, h;
		Flv_Style s;
		get_cell_bounds( x, y, w, h, row(), col() );
		position_editor(veditor, x, y, w, h, s );
		veditor->damage(FL_DAMAGE_ALL);
		veditor->draw();			//	So editor will overlay table
	}
}
...
int Flv_Table::internal_handle(int event)
{
	...
	switch( event )
	{
		...
		case FL_DRAG:
			...
			if (check_resize())
			{
//				if (edit_when()==FLV_EDIT_AUTOMATIC && veditor)
//				{
//					cancel_edit();
//					start_edit();
//				}
	...
	r = 0;	// DIK: local var r used without initializing
	switch(Fl::event_key())
	{
		case FL_Enter:
...
void Flv_Table::cancel_edit(void)											//	Cancel editing
{
	...
	edit_col = -1;
	vediting = false;
	switch_editor(edit_row, edit_col);
}
