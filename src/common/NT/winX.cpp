/* $Id: winX.cpp,v 1.11 2004/05/01 19:12:38 dick Exp $
 *
...
static void WinXCreateBitmapForXid(HWND hwnd, XID xidno, int cx, int cy)
{
	...
	if (iniClient.threadedDraw && xidno == draw)
	{
...
//		WinXScaled(hBmpDC, cx, cy);
	...
	FillRect(hBmpDC, &r, (HBRUSH)GetStockObject(BLACK_BRUSH));
...
//	max_xid = 1;
	...
	myLogPal = (LOGPALETTE*)malloc(sizeof(LOGPALETTE) + sizeof(PALETTEENTRY)*16);
...
//    "#9f9f9f", "#5f5f5f", "#dfdfdf", "#202020"
	...
	winmaxcolors = iniClient.maxColors;
	...
	if (winmaxcolors < 4)
		...
		winmaxcolors = 16;
	iniClient.maxColors = winmaxcolors;
...
void WinXShutdown()
{
	...
	for (i=0; i<(unsigned)iniClient.maxColors; i++)
	{
#ifdef PENS_OF_PLENTY
		if (objs[i].pen != NULL)
...
void WinXResize(void)
{
	...
	if (radar && (iniClient.instruments & SHOW_SLIDING_RADAR))
	{
...
void WinXExit()
{
	PostMessage(xid[top].hwnd.hWnd, WM_CLOSE, 0, 0);
}
...
Window WinXGetParent(Window w)
{
	...
	xid[txid].hwnd.hWnd = hwnd;			// the window that Windows found
